...
function roundReleasePseudoEpoch(resultDate, roundTime) {
  ...
  if (!base || !ROUNDS.includes(roundTime)) {
    return NaN;
  }
...
function isRoundReleased(resultDate, roundTime) {
  const release =
    roundReleasePseudoEpoch(resultDate, roundTime);

  return (
    Number.isFinite(release) &&
    myanmarPseudoEpoch() >= release
  );
}
...
function isResultPublic(item) {
  ...
  if (mode === "now" || item.published_at) {
    return true;
  }

  if (Number(item.auto_publish ?? 1) !== 1) {
    return false;
  }
...
function generatedRoundValues(resultDate, roundTime) {
  const h1 = stableHash(
    `${resultDate}|${roundTime}|SET`
  );

  const h2 = stableHash(
    `${resultDate}|${roundTime}|VALUE`
  );
...
async function ensureSchema(env) {
  ...
  await env.DB.prepare(`
    CREATE TABLE IF NOT EXISTS app_round_locks (
...
async function logAdmin(env, action, details = {}) {
  try {
    ...
    await env.DB.prepare(`
      INSERT INTO app_admin_logs(action, details)
      VALUES(?, ?)
    `)
...
async function logError(env, error, context = {}) {
  try {
    ...
    await env.DB.prepare(`
      INSERT INTO app_error_logs(message, context)
      VALUES(?, ?)
    `)
      .bind(
...
function getNextRoundInfo(
  ...
  for (const round of ROUNDS) {
    const releaseMs =
      roundReleasePseudoEpoch(date, round);
...
async function handleSaveResult(
  ...
  const roundTime =
    String(body.round_time || "").trim();

  const result2d =
    clean2D(body.result_2d);
  ...
  const publishMode =
    normaliseMode(body.publish_mode);
  ...
  if (!lock) {
    return json({
      success: false,
      error:
        "This round is temporarily locked. Please try again."
    }, 409);
  }
...
async function queryResultsForDate(
  env,
  date,
...
async function handleToday(url, env) {
  ...
  return json({
    ...
    nextRound: getNextRoundInfo(date),
    results
  });
}
...
async function fetchLiveMarket() {
  ...
  try {
    ...
    const cacheKey =
      new Request(
        "https://tartay.local/cache/btc-usd-ticker"
      );
    ...
    if (!response) {
      ...
      if (!upstream.ok) {
        throw new Error(
          `Market API HTTP ${upstream.status}`
        );
      }
...
async function lockReleasedMarketValues(
  env,
  date,
  market
) {
...
async function handleState(env) {
  ...
  if (
    ...
    preSpin = {
      active: true,
      result_date:
        nextRound.result_date,
      round_time:
        nextRound.round_time,
      seconds_until:
        nextRound.seconds_until,
      frame:
        buildPreSpinFrame(
          `${nextRound.result_date}|${nextRound.round_time}|${tick}`
        )
    };
  }
  ...
  return new Response(
    ...
    ),
    {
      headers: {
        ...
        "Content-Disposition":
          `attachment; filename="tartay-2d-backup-${getOperationalDate()}.json"`,
        "Cache-Control":
          "no-store"
      }
    }
...
function validateBackup(backup) {
  if (
    ...
    backup.appId !== "tartay-2d" ||
    !Array.isArray(
      backup.results
    )
  ) {
    throw new Error(
      "Invalid Tartay 2D backup file."
    );
  }
  ...
  for (
    ...
    if (
      ...
      throw new Error(
        "Backup contains invalid result data."
      );
    }
  }
}
...
async function handleRestore(
  ...
  for (
    const row of body.backup.results
  ) {
    await env.DB.prepare(`
      INSERT INTO app_results(
        result_date,
          ...
          JSON.stringify([
            {
              ...
              target: {
                ...
                package_name:
                  "dev.workers.yehtetaung7655.tartay_2d.twa",

                sha256_cert_fingerprints: [
                  "A2:2C:61:7A:C0:09:58:09:1B:A0:07:DD:53:07:69:AA:94:7E:D8:D7:87:40:7B:CE:1B:3E:5F:0E:19:6F:D4:F5"
                ]
              }
            }
          ]),

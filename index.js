...
const PUBLISH_BURST_INTERVAL_MS = 1000;
const PUBLISH_BURST_ATTEMPTS = 12;
...
async function acquireRoundLock(env, roundNumber) {
  ...
  // KV does not provide compare-and-set. The short settle/read-back step makes
  // concurrent writers converge on one winning token in normal operation.
  await delay(ROUND_LOCK_SETTLE_MS);
  const confirmed = await env.DATA.get(key);
...
async function releaseRoundLock(env, lock) {
  ...
  // KV reads can briefly be stale. Always remove the lock created by this
  // request so a schedule save cannot block the exact-time publisher for
  // nearly a minute.
...
async function publishRoundByNumberUnlocked(
  env,
  roundNumber,
  expectedDate = "",
  expectedTime = "",
  ...
  // Save the public state first so the exact-time result is visible immediately.
  await saveState(env, state);

  // History and logs are important, but they must not turn a successful publish
  // into a public API failure. Retryable failures are recorded separately.
  try {
  ...
  try {
    ...
    console.error("Publish log failed", error);
  }
...
async function publishRoundByNumber(
  ...
  // Every publish request goes through the round's Durable Object. Durable
  // Objects serialize requests, so no short-lived KV lock is needed and a
  // stale KV lock can no longer delay the result by about 60 seconds.
  try {
    const stub = getRoundAlarmStub(env, roundNumber);
    const response = await stub.fetch("https://round-alarm.internal/publish", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        source: String(source || "auto_publish")
      })
    });

    if (!response.ok) {
      throw new Error("Round publisher returned HTTP " + response.status);
    }

    return await response.json();
  } catch (error) {
    // Keep the app usable even if the Durable Object binding is temporarily
    // unavailable. The operation is idempotent because published rounds are
    // checked before writing.
    await logServerError(env, error, null, {
      source: "durable_publish_fallback",
      round: Number(roundNumber)
    });
    return publishRoundByNumberUnlocked(
      env,
      roundNumber,
      expectedDate,
      expectedTime,
...
async function autoPublish(env, source = "cron_fallback_publish") {
  ...
  for (let index = 0; index < TIMES.length; index++) {
    ...
    try {
      ...
      // A publication retry must never make /api/state fail and show a false
      // Connection Lost message. The next boundary retry or alarm will try again.
      await logServerError(env, error, null, {
        source: source + "_round_publish",
        round: index + 1
      });
    }
  }
...
export class RoundAlarm extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (request.method === "POST" && url.pathname === "/publish") {
      ...
      const result = await publishRoundByNumberUnlocked(
        this.env,
        Number(job.round),
        String(job.expectedDate || ""),
        String(job.expectedTime || ""),
        String(job.source || "durable_object_publish")
      );
  ...
  async alarm() {
    ...
    try {
      await publishRoundByNumberUnlocked(
        this.env,
        Number(job.round),
      ...
      // Retry quickly instead of leaving publication to a minute-late fallback.
...
.final-jump-once{animation:finalJumpOnce .58s cubic-bezier(.22,.82,.25,1) 1}
@keyframes finalJumpOnce{
  0%{transform:translateY(0) scale(1);filter:brightness(1)}
  34%{transform:translateY(-15px) scale(1.14);filter:brightness(1.58) drop-shadow(0 0 18px currentColor)}
  66%{transform:translateY(4px) scale(.96);filter:brightness(1.14)}
...
@keyframes resultBlink{0%,100%{opacity:1;transform:translateY(-2px) scale(1)}45%{opacity:.20;transform:translateY(-2px) scale(.96)}65%{opacity:1;transform:translateY(-2px) scale(1.025)}}
...
["bigResult","liveSet","liveValue"].forEach(function(id){
  document.getElementById(id).addEventListener("animationend",function(){
    this.classList.remove("blink-change","final-jump-once");
...
var PRE_SPIN_LEAD_MS=60*1000;
var PRE_SPIN_STEP_MS=10*1000;
...
var FINAL_JUMP_DURATION_MS=580;
var FINAL_JUMP_GAP_MS=220;
var FINAL_REVEAL_SETTLE_MS=(FINAL_JUMP_DURATION_MS*2)+FINAL_JUMP_GAP_MS+140;
var FINAL_REVEAL_RECENT_MS=120000;
var FINAL_REVEAL_SEEN_KEY="nz2d_final_reveal_seen_v3";
function finalRevealKey(round){
  if(!round){return ""}
  return [round.round,round.result,Number(round.publishedAtMs||0)].join("|");
}
function latestPublishedRound(rounds){
  return (Array.isArray(rounds)?rounds:[]).filter(function(round){
    return round&&round.status==="published"&&/^\d{2}$/.test(String(round.result||""));
  }).sort(function(a,b){return Number(b.publishedAtMs||0)-Number(a.publishedAtMs||0)})[0]||null;
}
function getSeenFinalRevealKey(){
  try{return String(sessionStorage.getItem(FINAL_REVEAL_SEEN_KEY)||"")}catch(error){return ""}
}
function setSeenFinalRevealKey(key){
  try{sessionStorage.setItem(FINAL_REVEAL_SEEN_KEY,String(key||""))}catch(error){}
}
function renderPublicRounds(rounds,hiddenKey){
  var grid=document.getElementById("roundGrid");
  if(!grid){return}
  grid.innerHTML=(Array.isArray(rounds)?rounds:[]).map(function(round,index){
    var key=finalRevealKey(round);
    var hidden=Boolean(hiddenKey&&key===hiddenKey);
    var published=!hidden&&round.status==="published"&&/^\d{2}$/.test(String(round.result||""));
    return '<article class="round-card tone-'+((index%6)+1)+'">'+
...
function triggerFinalJumpOnce(){
  var nodes=finalRevealNodes();
  if(!nodes.length){return}
  nodes.forEach(function(node){
    node.classList.remove("blink-change","pre-spin-change","final-jump-once");
    node.style.opacity="1";
  });
  void nodes[0].offsetWidth;
  nodes.forEach(function(node){node.classList.add("final-jump-once")});
}
function startFinalReveal(data,publishedRound){
  var key=finalRevealKey(publishedRound);
  if(!key||window.finalRevealRunning||window.finalRevealDoneKey===key||getSeenFinalRevealKey()===key){return false}
  window.finalRevealRunning=true;
  window.finalRevealKey=key;
  window.lastObservedPublishedKey=key;
  window.finalRevealLatestRounds=data.rounds||[];
  if(window.finalRevealTimers){window.finalRevealTimers.forEach(function(timer){clearTimeout(timer)})}
  window.finalRevealTimers=[];
  if(window.preSpinActive){stopPreSpin(false)}
  window.preSpinAwaitingPublish=false;

  // Put the real final SET / VALUE / 2D on screen, but keep the Round box hidden.
  updateBlink("bigResult",data.live.result||"--");
  updateBlink("liveSet",data.live.set||"--");
  updateBlink("liveValue",data.live.value||"--");
  renderPublicRounds(data.rounds,key);

  // Two separate jump animations. The second one is restarted only after the
  // first animation has fully ended, so both jumps are clearly visible.
  triggerFinalJumpOnce();
  window.finalRevealTimers.push(setTimeout(function(){
    triggerFinalJumpOnce();
  },FINAL_JUMP_DURATION_MS+FINAL_JUMP_GAP_MS));

  window.finalRevealTimers.push(setTimeout(function(){
    window.finalRevealRunning=false;
    window.finalRevealDoneKey=key;
    setSeenFinalRevealKey(key);
    finalRevealNodes().forEach(function(node){node.classList.remove("final-jump-once")});
    renderPublicRounds(window.finalRevealLatestRounds||data.rounds,"");
    scheduleFitPublicApp();
  },FINAL_REVEAL_SETTLE_MS));
  return true;
}
...
function exactPublishWatchdog(){
  var target=Number(window.currentNextAutoPublishAtMs||0);
  if(!target){return}
  var now=estimatedServerNow();
  // At and after the exact boundary, keep asking /api/state to publish the due
  // round. This guards against a delayed/missed mobile setTimeout or Durable
  // Object alarm. The endpoint only publishes rounds whose schedule is due.
  if(now>=target&&now<=target+2*60*1000&&!window.publishBurstRunning){
    publishBoundaryBurst(target);
  }
}
var CONNECTION_FAILURE_LIMIT=3;
window.connectionFailureCount=0;
...
async function loadApp(fromBoundaryBurst){
  try{
    ...
    var publishedRound=latestPublishedRound(data.rounds);
    var publishedKey=finalRevealKey(publishedRound);
    var previousObservedKey=String(window.lastObservedPublishedKey||"");
    var arrivedFromPreSpin=Boolean(window.preSpinActive||window.preSpinAwaitingPublish);
    var publishedAge=publishedRound?Math.max(0,Number(data.serverNow||Date.now())-Number(publishedRound.publishedAtMs||0)):Infinity;
    var freshPublish=publishedAge<=FINAL_REVEAL_RECENT_MS;
    var changedWhileOpen=Boolean(publishedKey&&previousObservedKey&&publishedKey!==previousObservedKey);
    var firstFreshLoad=Boolean(publishedKey&&!previousObservedKey&&freshPublish&&getSeenFinalRevealKey()!==publishedKey);
    var revealStarted=false;

    // Detect the actual waiting -> published change from the Round list itself.
    // This does not depend only on live.status, so the two jumps still run when
    // the response arrives late or the live override state changes quickly.
    if(publishedRound&&(arrivedFromPreSpin||changedWhileOpen||firstFreshLoad)){
      revealStarted=startFinalReveal(data,publishedRound);
    }
    if(publishedKey){window.lastObservedPublishedKey=publishedKey}
    if(livePublished&&!revealStarted&&!window.finalRevealRunning){
      if(window.preSpinActive){stopPreSpin(false)}
      window.preSpinAwaitingPublish=false;
    }
    if(window.finalRevealRunning){
      window.finalRevealLatestRounds=data.rounds||[];
      updateBlink("bigResult",data.live.result||"--");
      updateBlink("liveSet",data.live.set||"--");
      updateBlink("liveValue",data.live.value||"--");
    }else if((!window.preSpinActive&&!window.preSpinAwaitingPublish)||livePublished){
      updateBlink("bigResult",data.live.result||"--");
      updateBlink("liveSet",data.live.set||"--");
      updateBlink("liveValue",data.live.value||"--");
    }
    ...
    renderPublicRounds(data.rounds,window.finalRevealRunning?window.finalRevealKey:"");
    scheduleFitPublicApp();
  }catch(error){
    // Exact-publish burst retries happen several times within a few seconds.
    // Do not count those rapid internal retries as separate connection losses.
    if(!fromBoundaryBurst){
      window.connectionFailureCount=Number(window.connectionFailureCount||0)+1;
    }

    if(Number(window.connectionFailureCount||0)>=CONNECTION_FAILURE_LIMIT){
    ...
    // For the first two normal failures, keep the last successful LIVE/PAUSED
    // state and displayed data. A single short mobile-network interruption will
    // therefore not flash OFFLINE on the screen.
    console.warn("App data retry",window.connectionFailureCount,error);
  }
}
...
setInterval(exactPublishWatchdog,250);
</script>
</body>
</html>`;
...
<section class="card">
  ...
  <button type="button" data-admin-action class="orange" onclick="clearServerErrorLogs()">Clear Server Error Logs</button>
...
async function clearServerErrorLogs(){
  if(!confirm("Server Error Logs အဟောင်းများအားလုံးကို ဖျက်မည်။ ဆက်လုပ်မလား?")){return}
  var data=await secureRequest("/api/admin/errors/clear",{},
    {reload:false,loading:"Server Error Logs ရှင်းနေပါတယ်..."});
  if(data&&data.success){
    el("logBox").textContent="Server Error Logs ရှင်းပြီးပါပြီ။ ဖျက်ထားသည့် Log အရေအတွက်: "+String(data.deleted||0);
  }
}
...
async function handleAdminRoute(request, env, url) {
  ...
  if (url.pathname === "/api/admin/errors/clear") {
    ...
    await logAdminAction(env, request, "server_error_logs_cleared", {
      deleted
    });

    return jsonResponse({
      success: true,
      message: "Server Error Logs အားလုံးရှင်းပြီးပါပြီ",
      deleted
    });
  }
  ...
  if (roundMatch) {
    ...
    // Saving a schedule must never create a KV lock that can hold the exact-time
    // publisher until the lock TTL expires.
    const lock = null;
...
async function handleRequest(request, env) {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/state") {
      ...
      let state;
      try {
        state = await autoPublish(env, "state_boundary_publish");
      } catch (error) {
        await logServerError(env, error, request, {
          source: "state_boundary_publish_fallback"
        });
        state = await getState(env);
      }
      const live = getLiveData(state.live);
...
async function scheduledMaintenance(env) {
  const state = await getState(env);
  await syncAllRoundAlarms(env, state);
